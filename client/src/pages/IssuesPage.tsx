      ...
      >
        <CardContent className="p-0">
          ...
          <div className="flex items-center justify-between p-4 sm:p-6 pb-3 sm:pb-4">
            <div 
              className="flex items-center space-x-2 sm:space-x-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg p-2 -m-2 transition-colors flex-1 min-w-0"
              onClick={(e) => {
                e.stopPropagation();
            ...
            >
              <div className="relative flex-shrink-0">
                <Avatar className="w-10 h-10 sm:w-12 sm:h-12 ring-2 ring-blue-200 dark:ring-blue-800">
                  ...
                  <AvatarFallback className="text-xs sm:text-sm font-semibold bg-gradient-to-r from-blue-500 to-orange-500 text-white">
                    {issue.authorName?.charAt(0) || 'U'}
                  </AvatarFallback>
                </Avatar>
                <div className="absolute -bottom-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 bg-green-500 rounded-full border-2 border-white dark:border-slate-800"></div>
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm sm:text-base hover:text-blue-600 dark:hover:text-blue-400 transition-colors truncate">
                  {issue.authorName || 'Community Leader'}
                </h3>
                <div className="flex items-center text-xs sm:text-sm text-slate-500 dark:text-slate-400 flex-wrap gap-1">
          ...
          <div className="px-3 sm:px-4 pb-2 sm:pb-3">
            <h2 className="text-sm sm:text-base font-semibold text-card-foreground mb-2">{issue.title}</h2>
            <p className="text-muted-foreground text-xs sm:text-sm leading-relaxed line-clamp-3 sm:line-clamp-none">{issue.description}</p>
          </div>
          ...
          <div className="px-3 sm:px-4 py-2 sm:py-3 border-t border-border">
            <div className="flex items-center justify-between gap-2">
              ...
              <Button
                ...
                className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-muted-foreground hover:text-primary"
              >
              ...
              <Button
                ...
                className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-muted-foreground hover:text-accent-foreground"
              >
...
export default function IssuesPage() {
      ...
      <div className="space-y-3">
        ...
        <div className="flex space-x-2 sm:space-x-3">
          ...
          <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center flex-wrap gap-1 sm:gap-2 mb-1">
                ...
                {isOwner && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-xs text-red-500 hover:text-red-700 p-0 h-auto ml-auto"
            ...
            <div className="flex items-center flex-wrap gap-2 sm:gap-4 mt-2 ml-2 sm:ml-4">
              <Button 
                variant="ghost" 
                size="sm" 
                className={`text-xs ${isLiked ? 'text-red-500' : 'text-slate-500 hover:text-red-500'} h-7`}
                onClick={toggleLike}
                disabled={likeLoading}
              >
                <ThumbsUp className={`w-3 h-3 mr-1 ${isLiked ? 'fill-current' : ''}`} />
                <span className="hidden sm:inline">Like</span> ({comment.likesCount || 0})
              </Button>
              <Button 
                variant="ghost" 
                size="sm" 
                className="text-xs text-slate-500 hover:text-blue-500 h-7"
              ...
              {comment.repliesCount > 0 && (
                ...
                >
                  ...
                  <span className="hidden sm:inline">{showRepliesForThis ? 'Hide' : 'View'} </span>
  ...
  return (
    <div className="md:ml-64 min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
          ...
          >
            <Card className="bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm border-slate-200 dark:border-slate-700 shadow-xl">
              <CardContent className="p-8 sm:p-12 text-center">
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-r from-blue-100 to-orange-100 dark:from-blue-900/30 dark:to-orange-900/30 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                  <AlertTriangle className="w-8 h-8 sm:w-10 sm:h-10 text-blue-600 dark:text-blue-400" />
                </div>
                <h3 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-200 mb-2 sm:mb-3">No Issues Yet</h3>
                <p className="text-sm sm:text-lg text-slate-600 dark:text-slate-400">Be the first to report a community issue and help make a difference.</p>
              </CardContent>
            </Card>
          </motion.div>
        ) : (
            ...
            <div className="flex flex-col h-[90vh]">
              ...
              <div className="flex-1 overflow-y-auto min-h-0">
                ...
                <div className="p-4 sm:p-6">
                  ...
                  {/* Post Actions */}
                  <div className="flex items-center justify-between py-3 sm:py-4 border-t border-slate-200 dark:border-slate-700 gap-2">
                    <Button 
                      variant="ghost" 
                      size="sm"
                      className={`flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm ${isPostLiked ? 'text-red-500' : 'text-slate-600 dark:text-slate-400 hover:text-red-500'}`}
                    ...
                    <Button variant="ghost" size="sm" className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-slate-600 dark:text-slate-400 hover:text-blue-500">
                      <MessageCircle className="w-4 h-4 sm:w-5 sm:h-5" />
                      <span>{openIssue.commentsCount || 0}</span>
                    </Button>
                    <Button variant="ghost" size="sm" className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-slate-600 dark:text-slate-400 hover:text-green-500">
                ...
                <div className="px-4 sm:px-6 pb-4 sm:pb-6">
                  ...
                  <div className="flex space-x-2 sm:space-x-3">
                    <Avatar className="w-7 h-7 sm:w-8 sm:h-8 flex-shrink-0">
                      <AvatarImage src={currentUser?.profileImage} />
                      <AvatarFallback className="bg-gradient-to-r from-blue-500 to-orange-500 text-white text-xs">
                        {currentUser?.firstName?.charAt(0) || 'U'}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 flex space-x-2">
